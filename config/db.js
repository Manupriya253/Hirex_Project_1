...
const Connected = new Promise((resolve, reject) => {
  ...
  rootConnection.connect((err) => {
    ...
    // Skip CREATE DATABASE if using shared hosting like cPanel (DB already created manually)
    ...
    db.connect((err) => {
      if (err) return reject("Failed to connect to DB: " + err.message);
      ...
      db.query(createEmployeeTable, (err) => {
        ...
        db.query(createAssetsTable, (err) => {
          if (err) return reject("Failed to create 'employee_assets' table: " + err.message);
